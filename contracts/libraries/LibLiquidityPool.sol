...
library LibLiquidityPool {
    ...
    function _deposit(
        ...
        if (!_appStorage.s_protocolPool[_token].initialize) revert ProtocolPool__NotInitialized();

        if (_amount == 0) revert ProtocolPool__ZeroAmount();
        if (!_appStorage.s_isLoanable[_token]) revert ProtocolPool__TokenNotSupported();
        if (!_appStorage.s_protocolPool[_token].isActive) revert ProtocolPool__IsNotActive();
        ...
        // Mint VToken Vault shares if vault exists
        address vaultAddress = _appStorage.s_vaults[_token];
        if (vaultAddress == address(0)) revert ProtocolPool__VaultNotDeployed();
    ...
    function _borrowFromPool(
        ...
        if (_protocolPool.totalBorrows + _amount > _protocolPool.totalSupply) revert ProtocolPool__NotEnoughLiquidity();

        if (!_appStorage.s_protocolPool[_token].isActive) revert ProtocolPool__IsNotActive();

        if (tokenData.poolLiquidity < _amount) revert ProtocolPool__NotEnoughLiquidity();
    ...
    function _repay(
        ...
        if (!userBorrowData.isActive || userBorrowData.borrowedAmount == 0) revert ProtocolPool__NoBorrow();
    ...
    function _lockCollateral(
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            ...
            uint256 availableBalance = _appStorage.s_addressToAvailableBalance[_user][token];
            if (amountToLock > availableBalance) amountToLock = availableBalance;
    ...
    function _unlockCollateral(
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            ...
            if (amountToUnlock > lockedAmount) amountToUnlock = lockedAmount;
