...
contract SharedFacetTest is Base {
    ...
    function test_depositCollateral() public {
        ...
        uint256 userBalance = gettersFacet.getAddressToCollateralDeposited(owner, USDT_CONTRACT_ADDRESS);

        assertEq(userBalance, amount);
        assertEq(ERC20Mock(USDT_CONTRACT_ADDRESS).balanceOf(address(sharedFacet)), amount);
    }
    ...
    function test_withdrawCollateral() public {
        ...
        uint256 afterWithdrawBalance = gettersFacet.getAddressToCollateralDeposited(owner, USDT_CONTRACT_ADDRESS);

        assertEq(afterWithdrawBalance, 0);
        assertEq(ERC20Mock(USDT_CONTRACT_ADDRESS).balanceOf(address(sharedFacet)), 0);
    }
