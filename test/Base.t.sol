...
contract Base is Test, IDiamondCut {
    ...
    address WETH_CONTRACT_ADDRESS = address(0xbAd49269309C439e811E2315B343c4b54CdBEd07);
    ...
    address ARB_LINK_CONTRACT_ADDRESS = address(0x23eb68D3C0472f6892c2d68B0F2A8F0f5282a7ED);
    address ARB_USDT_CONTRACT_ADDRESS = address(0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d);
    address ARB_WETH_CONTRACT_ADDRESS = address(0x5cB4bfA9f8803Ae9231b6eAC4AC85e1dAd8bC432);
    address AVAX_LINK_CONTRACT_ADDRESS = address(0xdEd13e60DF60b5C1CA192160487D7B5ef82769A4);
    address AVAX_USDT_CONTRACT_ADDRESS = address(0x5425890298aed601595a70AB815c96711a31Bc65);
    ...
    function deployXDiamonds() public virtual {
        ...
        string memory SPOKE_ARB_RPC_URL = vm.envString("ARBITRUM_SEPOLIA_RPC_URL");
        ...
        LINK_CONTRACT_ADDRESS = address(0x46d4AafcEd9cc65089D1606e6cAE85fe6D7df456);
        USDT_CONTRACT_ADDRESS = address(0x036CbD53842c5426634e7929541eC2318f3dCF7e); //Here we are using a USDC address instead of USDT.
        ...
        Register.NetworkDetails memory arbNetworkDetails = ccipLocalSimulatorFork.getNetworkDetails(block.chainid);

        (ABR_USDT_ADDRESS_ON_SPOKE, USDT_USD) = deployERC20ContractAndAddPriceFeed("USDT", 6, 1);

        (ABR_LINK_ADDRESS_ON_SPOKE, LINK_USD) = deployERC20ContractAndAddPriceFeed("LINK", 18, 10);
        ...
        arbSpokeContract.addToken(ARB_USDT_CONTRACT_ADDRESS, USDT_CONTRACT_ADDRESS);

        arbSpokeContract.addToken(ARB_LINK_CONTRACT_ADDRESS, LINK_CONTRACT_ADDRESS);

        arbSpokeContract.addToken(ARB_WETH_CONTRACT_ADDRESS, WETH_CONTRACT_ADDRESS);
        ...
        Register.NetworkDetails memory avaxNetworkDetails = ccipLocalSimulatorFork.getNetworkDetails(block.chainid);

        (AVAX_USDT_ADDRESS_ON_SPOKE, USDT_USD) = deployERC20ContractAndAddPriceFeed("USDT", 6, 1);

        (AVAX_LINK_ADDRESS_ON_SPOKE, LINK_USD) = deployERC20ContractAndAddPriceFeed("USDT", 18, 1);
        ...
        avaxSpokeContract.addToken(AVAX_USDT_CONTRACT_ADDRESS, USDT_CONTRACT_ADDRESS);
        avaxSpokeContract.addToken(AVAX_LINK_CONTRACT_ADDRESS, LINK_CONTRACT_ADDRESS);
        avaxSpokeContract.addToken(avaxNetworkDetails.wrappedNativeAddress, WETH_CONTRACT_ADDRESS);
        avaxSpokeContract.addToken(ETH_CONTRACT_ADDRESS, ETH_CONTRACT_ADDRESS);

        avaxSpokeContract.addUncrossedToken(AVAX_USDT_ADDRESS_ON_SPOKE);
        avaxSpokeContract.addUncrossedToken(AVAX_LINK_ADDRESS_ON_SPOKE);

        vm.selectFork(hubFork);
        ownerF.addSupportedChain(arbNetworkDetails.chainSelector, address(arbSpokeContract));
        ownerF.addSupportedChain(avaxNetworkDetails.chainSelector, address(avaxSpokeContract));

        ccipLocalSimulatorFork.requestLinkFromFaucet(address(diamond), 1000000 ether);
    }
        ...
        returns (address, address)
    {
        ...
        MockV3Aggregator priceFeed = new MockV3Aggregator(_decimals, _initialAnswer * 1e8);
    ...
    function xdepositIntoLiquidityPool(address _token, uint256 _amount, uint256 _fork, address _user) public {
    ...
    function _xWithdrawnFromPool(address _token, uint256 _amount, uint256 _fork, address _user) public {
        if (_fork == hubFork) {
    ...
    function _xborrowFromPool(address _token, uint256 _amount, uint256 _fork, address _user) public {
        if (_fork == hubFork) {
    ...
    function xRepayFromPool(address _token, uint256 _amount, uint256 _fork, address _user) public {
        if (_token == ETH_CONTRACT_ADDRESS) {
            revert("ETH is not supported use _xDepositNativeCollateral");
        }
    ...
    /////////////////////////
    /////////////////////////
    function _xDepositCollateral(address _token, uint256 _amount, uint256 _fork, address _user) public {
        if (_token == ETH_CONTRACT_ADDRESS) {
            revert("ETH is not supported use _xDepositNativeCollateral");
        }
        if (_fork == hubFork) {
        ...
        if (_fork == avaxFork) {
            ...
            vm.deal(_user, 1 ether);
            ERC20Mock(_token).approve(address(avaxSpokeContract), _amount);
            avaxSpokeContract.depositCollateral{value: 1 ether}(_token, _amount);
        }
        //give ccipLocalSimulatorFork the ability to route messages
        ccipLocalSimulatorFork.switchChainAndRouteMessage(hubFork);
    }

    function _xDepositNativeCollateral(address _user, uint256 _amount, uint256 _fork) public {
        if (_fork == hubFork) {
        ...
        if (_fork == arbFork) {
            ...
            arbSpokeContract.depositCollateral{value: _amount + 1 ether}(ETH_CONTRACT_ADDRESS, _amount);
        }

        if (_fork == avaxFork) {
            vm.selectFork(_fork);
            vm.deal(_user, _amount + 1 ether);
            avaxSpokeContract.depositCollateral{value: _amount + 1 ether}(ETH_CONTRACT_ADDRESS, _amount);
        }
        //give ccipLocalSimulatorFork the ability to route messages
        ccipLocalSimulatorFork.switchChainAndRouteMessage(hubFork);
    }

    function _xWithdrawCollateral(address _token, uint256 _amount, uint256 _fork, address _user) public {
        if (_fork == hubFork) {
        ...
        if (_fork == arbFork) {
            vm.selectFork(_fork);
            vm.deal(_user, 1 ether);
            arbSpokeContract.withdrawCollateral{value: 1 ether}(_token, _amount);
        }

        if (_fork == avaxFork) {
            vm.selectFork(_fork);
            vm.deal(_user, 1 ether);
            avaxSpokeContract.withdrawCollateral{value: 1 ether}(_token, _amount);
        }
        //give ccipLocalSimulatorFork the ability to route messages
        ccipLocalSimulatorFork.switchChainAndRouteMessage(hubFork);
        ccipLocalSimulatorFork.switchChainAndRouteMessage(_fork);
    ...
    function _depositNativeCollateral(address _user, uint256 _amount) public {
        ...
        sharedFacet.depositCollateral{value: _amount}(ETH_CONTRACT_ADDRESS, _amount);
    }
