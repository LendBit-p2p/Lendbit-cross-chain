...
contract LiquidityPoolFacetTest is Base {
    ...
    function test_depositInto_LiquidityPool() public {
        ...
        uint256 userBalance = gettersFacet.getUserPoolDeposit(owner, USDT_CONTRACT_ADDRESS);

        assertEq(userBalance, amount);
        assertEq(ERC20Mock(USDT_CONTRACT_ADDRESS).balanceOf(address(liquidityPoolFacet)), amount);
    }
    ...
    function test_withdrawFrom_LiquidityPool() public {
        ...
        liquidityPoolFacet.withdraw(USDT_CONTRACT_ADDRESS, amount);
        uint256 afterWithdrawBalance = gettersFacet.getUserPoolDeposit(owner, USDT_CONTRACT_ADDRESS);
    ...
    function testBorrowFromPool() public {
        ...
        uint256 userBalance = gettersFacet.getAddressToCollateralDeposited(user, LINK_CONTRACT_ADDRESS);

        liquidityPoolFacet.borrowFromPool(USDT_CONTRACT_ADDRESS, amountToBorrowed);

        vm.stopPrank();

        (uint256 borrowedAmount,,, bool isActive) = gettersFacet.getUserBorrowData(user, USDT_CONTRACT_ADDRESS);
    ...
    function testRepayToLoanToPool() public {
        ...
        uint256 currentDebt = gettersFacet.getUserDebt(user, USDT_CONTRACT_ADDRESS);
        ...
        (uint256 remainingBorrowedAmount,,, bool isStillActive) =
            gettersFacet.getUserBorrowData(user, USDT_CONTRACT_ADDRESS);
